...
const placeholder = { "english-to-chines-dictionary": "Search over 1 million Words, Records, and Synonyms with", network_security_terms: "Search Over 897 Network Security Terms and Definitions" }
    ...
    function searchTerms() {
        ...
        async function transformList({
            ...
            if (target === document.querySelector('form > span > a:hover')) {
                element.innerHTML = `<span> ${queryResult[target.id].word}  :  ${(queryResult[target.id].definition || "" ) + "<br/>" + queryResult[target.id].translation} </span>`
                const placeholder = document.getElementById('complete')
